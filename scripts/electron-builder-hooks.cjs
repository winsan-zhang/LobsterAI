...
function resolveOpenClawNpmRoot() {
  const npmPath = path.join(__dirname, '..', 'node_modules', 'openclaw');
  return existsSync(npmPath) ? npmPath : null;
}

function ensureBundledOpenClawRuntime(context) {
  const runtimeRoot = resolveOpenClawNpmRoot();
  if (!runtimeRoot) {
    throw new Error(
      '[electron-builder-hooks] OpenClaw npm package not found. Run `npm install` first.',
    );
  }

  // Sync local extensions (e.g., mcp-bridge) into the npm package
  ...
  console.log(`[electron-builder-hooks] Verified OpenClaw npm package at: ${runtimeRoot}`);
}
...
/**
 * Clean up broken symlinks in OpenClaw extensions to prevent macOS signing failures.
 */
function cleanupBrokenSymlinksInExtensions(appOutDir) {
  // Check both new npm location and legacy cfmind location
  const candidates = [
    path.join(appOutDir, 'Contents', 'Resources', 'app.asar.unpacked', 'node_modules', 'openclaw', 'extensions'),
    path.join(appOutDir, 'Contents', 'Resources', 'cfmind', 'extensions'),
  ];
  ...
  console.log(`[electron-builder-hooks] Cleaning up broken symlinks in ${path.basename(path.dirname(extensionsDir))}/extensions...`);
