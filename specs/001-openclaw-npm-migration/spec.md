...
- **FR-002**: System MUST bundle the OpenClaw runtime into the Electron asar archive, using `asarUnpack` for native binary components (auto-extracted to `app.asar.unpacked/` at build time)
...
- **FR-005**: System MUST handle platform-specific native binaries correctly via a single npm package (version 2026.3.13) that downloads the appropriate platform binary during post-install (similar to the esbuild distribution model)
- **FR-006**: System MUST preserve the ability to load and use local extensions (e.g., MCP bridge from `openclaw-extensions/`)
- **FR-007**: System MUST preserve the ability to load plugins (DingTalk, Feishu, WeCom, QQBot) as separate npm dependencies in package.json, replacing the current vendor directory approach
...
- **FR-009**: System MUST remove or deprecate the source-compilation build scripts and patch mechanism that are no longer needed (patches are pre-applied in the npm package)
...
- **OpenClaw Plugins**: Third-party IM connector plugins, migrated from vendor directory to separate npm dependencies in package.json
...
- Q: Does the OpenClaw npm package already exist or need to be created? → A: Package already exists or will be published separately as a prerequisite; not in scope for this feature.
- Q: How should native binaries be handled within the asar archive? → A: Use Electron's `asarUnpack` to auto-extract OpenClaw native components at build time; JS modules stay in asar, native binaries go to `app.asar.unpacked/`.
- Q: What happens to the gateway-entry patch with the npm package? → A: Patches are pre-applied in the npm package (upstream responsibility); no patch mechanism needed in LobsterAI.
- Q: How should plugins be distributed after migration? → A: Plugins remain as separate npm dependencies in package.json (moved from vendor directory), with independent version management.

## Assumptions

- An official OpenClaw npm package exists and is published as a prerequisite (out of scope for this feature)
